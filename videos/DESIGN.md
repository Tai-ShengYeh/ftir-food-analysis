# DESIGN — 紅外光譜食品分析教學影片（PCA / PLS-DA 共用視覺系統）
...
> 資料：Mendeley Data **frrv2yd9rg** Instant-Coffee MIR-DRIFT FTIR（56 支即溶咖啡 × 286 波數，Arabica vs Robusta，公開）。
...
指向本地 `assets/fonts/*.otf`（GOTCHAS C-5）。技術名詞（FTIR / DRIFT / PCA / PLS-DA / SNV / cm⁻¹ / LV）
以拉丁字母呈現，源石黑體本身含 Latin 字符，數字與英文一併使用。

| 字重 | 用途 |
|------|------|
| **H (900)** | 封面標題、章節大標、關鍵數字（100% / 2 LV / 286） |
| **B (700)** | 副標、章節編號、字幕 |
| **M (500)** | 內文、定義、註解 |
...
| `#0E7C7B` | **teal** | 主強調：PC1、Arabica、訓練、概念名詞 |
| `#E36414` | **coral** | 副強調：關鍵數字、Robusta、轉折點、回歸係數 |
| `#C8941F` | gold | 第三色，僅用於微點綴（強調數字、≤2 處 / 頁） |

> 所有 matplotlib 圖表（ft01–ft09）已用同一組色票輸出，影片與圖表色感一致。
> 品種色票：Arabica=teal `#0E7C7B`、Robusta=coral `#E36414`。
...
| **T1** | 大圖置中 + 標題列 | 展示 matplotlib 圖表（光譜、scree、分數圖、混淆矩陣…） |
| **T2** | 左圖右文 / 左文右圖 | 圖＋解說並列 |
| **T3** | SVG 概念動畫置中 | FTIR 測量原理、SNV 前處理、PCA 旋轉軸、PCA↔PLS-DA、知識地圖 |
...
**PCA 片（一）**
1. **每支咖啡=286 點光譜聚合**（T3）：多條原始光譜折線在螢幕上並列，聚焦收束成 2D 平面上的一個點 → 降維直覺。
2. **SNV 前後對比動畫**（T2 疊 ft01→ft02）：原始光譜有基線漂移，SNV 後各品種分帶清晰，crossfade 顯示前後差異。
3. **點雲旋轉找軸**（T3）：散布點雲，PC1 沿最大分散方向畫出，PC2 垂直彈出；品種色 teal / coral。
4. **品種染色 aha 時刻**（T1 疊 ft04）：點雲先灰色，再由灰轉品種色，Arabica 左、Robusta 右自動分群，凸顯非監督下品種浮現。
5. **知識地圖輻射**（T3）：光譜 → SNV 前處理 → PCA → 分數 / 負荷量 / 陡坡 中心輻射。

**PLS-DA 片（二）**
1. **PCA vs PLS-DA 雙箭頭**（T3）：一箭頭追 X 最大分散（非監督），另一箭頭「轉向」對齊 Arabica/Robusta 分界（監督）。
2. **CV 曲線繪製 + 轉折脈動**（T1 疊 ft07）：準確率曲線左到右畫出，2 LV 最佳點 coral 放大脈動；標記 1 LV 已 98.2%。
3. **混淆矩陣填格**（T1 疊 ft08）：對角 29 / 27 逐格填入、亮綠，100% 命中。
4. **鑑別波段長條長出 → 連回咖啡化學**（T1 疊 ft09）：1007 / 957 / 903 cm⁻¹ 長條高亮 → 醣類 C–O → Arabica / Robusta 真實化學差異。
...
- **字幕**：單行 ≤ 25 字、不換行（GOTCHAS B-1）；技術名詞保留英文（PCA / PLS-DA / FTIR / SNV / LV / cm⁻¹）
