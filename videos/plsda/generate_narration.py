...
SCRIPT = [
    ...
    (10, "光譜把咖啡分開了，但這是真的化學差異，還是光譜的巧合？我們換一份完全不同的資料來檢驗：pgmm 套件裡的咖啡資料，四十三支咖啡，直接用濕式化學量了十二種成分，脂肪、咖啡因、葫蘆巴鹼、綠原酸。光是對這些成分做 PCA，Arabica 和 Robusta 一樣自己分成兩群。"),
    (11, "哪些成分在分？脂肪和葫蘆巴鹼，Arabica 明顯比較高；咖啡因和綠原酸，Robusta 比較高。這些，正是咖啡學裡公認的品種標記，而它們在紅外光譜裡都有對應的吸收。用這份真實成分做 PLS-DA，交叉驗證一樣是百分之百。兩份完全不同的資料、同一個結論：FTIR 看到的差異，是真實的咖啡化學。"),
    (12, "把品種換成別的標籤，同一套 PLS-DA，就是食品真偽鑑別的主力工具：判斷咖啡有沒有摻入廉價 Robusta、蜂蜜有沒有摻糖、橄欖油的產地。FTIR 給指紋；PLS-DA 給答案。"),
    (13, "最後，把兩支影片連起來。同樣的 FTIR 光譜、同樣的 SNV 前處理，分成兩條路：想探索、找結構，用 PCA，非監督；想鑑別、做決定，用 PLS-DA，監督。先探索，再建模，這就是食品 FTIR 分析的完整地圖。"),
    (14, "一句話帶走：PLS-DA，讓 FTIR 光譜指紋，直接說出，這，是 Arabica，還是 Robusta。"),
    (15, "看完概念，下一步，就是打開 Orange Data Mining，不用寫一行程式，親手把這整套 PCA 和 PLS-DA 拉一遍。我們，課堂見。"),
]
async def synth(i,t):
